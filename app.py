...
# CONFIGURAÇÃO EXCLUSIVA VELO PLAY
VELO_SERVER = "http://fastcdn.bond:8080" 
CREDENTIAL = "vWRJvKkPDX" # Seu código exato (Case Sensitive)

def get_veloplay_list():
    """
    Captura a lista completa de canais (incluindo Premium) 
    direto do servidor fastcdn.bond usando sua credencial.
    """
    # Usando seu código para Usuário e Senha (padrão Xtream Codes)
    url = f"{VELO_SERVER}/get.php?username={CREDENTIAL}&password={CREDENTIAL}&type=m3u_plus&output=ts"
    
    headers = {
        'User-Agent': 'IPTVSmartersPlayer', # Simula um player para evitar bloqueios
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Adiciona o link do EPG dinâmico do Railway no cabeçalho da lista
            ...
            return f"#EXTM3U\n#ERRO: Servidor Velo Play recusou a conexão (Status {response.status_code})"
    except Exception as e:
        return f"#EXTM3U\n#ERRO: Falha ao conectar no servidor fastcdn.bond\n#{str(e)}"
...
@app.route('/epg.xml')
def epg():
    # Puxa o Guia de Programação oficial do Velo Play usando sua credencial
